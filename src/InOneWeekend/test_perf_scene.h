...
void Create_test_perf_scene(hittable_list& InOutWorld, std::vector<std::shared_ptr<material>> InOutMaterials, camera& InOutCamera)
{
    ...
    auto GetColorVector3 = [&nextIndex]() -> vec3 {return vec3( fixedDistrib_0_1_441[nextIndex()], fixedDistrib_0_1_441[nextIndex()], fixedDistrib_0_1_441[nextIndex()]); };
    ...
    for (int a = -11; a < 11; a++) {
        for (int b = -11; b < 11; b++) {
            ...
            if ((center - point3(4, 0.2, 0)).length() > 0.9) {
                ...
                if (choose_mat < 0.8) {
                    // diffuse
                    auto albedo = GetColorVector3() * GetColorVector3();
                    sphere_material = make_shared<lambertian>(albedo);
                    InOutWorld.add(make_shared<sphere>(center, 0.2, sphere_material));
                }
                else if (choose_mat < 0.95) {
                    // metal
                    auto albedo = (GetColorVector3() * 0.5) + vec3(0.5, 0.5, 0.5);
                    auto fuzz = GetDouble() * 0.5;
    ...
    InOutCamera.image_width = 400;//1200//400// (2160);
